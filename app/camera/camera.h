...
#define IMG_IRQ_Pin 	GPIO_Pin16
#define CameraPT_PTx	PTC
#define CameraPT_Pin	GPIO_Pin0_7
#define DMA_REQ			PORTD_DMAREQ
#define DMA_PTx			PTD
#define DMA_Pin			GPIO_Pin12 // PCLK
#define LINE_IRQ_PTx	PTB
#define LINE_IRQ_Pin	GPIO_Pin20
...
typedef struct
{
	...
	void (*Extract)(void);
	void (*DiffExtract)(void);

	int isReady;
	int white;
	int black;
	int cnst;
	uint8 buf1[ROW + 10][COLUMN / 8], buf2[ROW + 10][COLUMN / 8];
	uint8 img[ROW][COLUMN];
} CAMERA_CLASS;
